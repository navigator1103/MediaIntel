...
interface TvTargetAudienceFormProps {
  countryId: number;
...
export default function TvTargetAudienceForm({
  countryId,
  businessUnitId,
  audiences,
  onAudiencesChange,
  ...
  React.useEffect(() => {
    const checkExistingData = async () => {
      ...
      try {
        const response = await fetch(`/api/admin/tv-diminishing-returns/audiences?countryId=${countryId}&businessUnitId=${businessUnitId}`);
        if (response.ok) {
  ...
  // Function moved to parent component
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        <div className="px-6 py-4 border-b border-gray-200">
          <div>
            <h3 className="text-lg font-medium text-gray-900">TV Target Audiences</h3>
            <p className="text-sm text-gray-500 mt-1">
              Define the target audiences for your TV diminishing returns curves.
            </p>
          </div>
        </div>
                  ...
                  <tr key={audience.id}>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <input
                        ...
                        className="block w-32 text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 bg-gray-50"
