...
interface MultimediaCurvesTableProps {
  countryId: number;
...
export default function MultimediaCurvesTable({
  countryId,
  businessUnitId,
  audiences,
  onSave
}: MultimediaCurvesTableProps) {
  ...
  const loadExistingData = async () => {
    ...
    try {
      // Load curves data
      const curvesResponse = await fetch(`/api/admin/multimedia-diminishing-returns/curves?countryId=${countryId}&businessUnitId=${businessUnitId}`);
      ...
      const audienceResponse = await fetch(`/api/admin/multimedia-diminishing-returns/audiences?countryId=${countryId}&businessUnitId=${businessUnitId}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const audienceResponse = await fetch('/api/admin/multimedia-diminishing-returns/audiences', {
        method: 'POST',
      ...
      // Save curves data
      const curvesResponse = await fetch('/api/admin/multimedia-diminishing-returns/curves', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          countryId,
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
          ...
          <div className="flex space-x-2">
            ...
            <button
              ...
              className="inline-flex items-center px-3 py-2 border-2 border-purple-300 shadow-md text-sm leading-4 font-semibold rounded-md text-purple-700 bg-purple-50 hover:bg-purple-100 hover:border-purple-400 transition-colors"
            >
        ...
        <div className="p-6">
          ...
          <div className="mb-4 grid gap-3" style={{ gridTemplateColumns: `repeat(${audiences.length}, 1fr)` }}>
            {audiences.map((audience, index) => {
              ...
              return (
                <div key={audience.id} className="bg-purple-50 rounded-lg p-3 text-center border border-purple-200">
                  <div className="font-semibold text-purple-900 mb-1">{audience.finalTarget}</div>
                  ...
                  {index > 0 && (
                    <button
                      ...
                      className="text-purple-700 hover:text-purple-800 text-xs mt-2 flex items-center justify-center mx-auto"
                      ...
                      return (
                        <div key={audience.id} className="border-r border-gray-300 p-0 relative">
                          <input
                            ...
                            className={`w-full h-10 px-3 text-sm text-center border-0 focus:ring-2 focus:ring-purple-500 focus:outline-none ${
