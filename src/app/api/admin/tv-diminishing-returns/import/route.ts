...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < importData.length; i += batchSize) {
      ...
      await prisma.tvDiminishingReturns.createMany({
        data: batch,
        skipDuplicates: true
      });
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully imported ${importedCount} TV Diminishing Returns records`,
      importedCount,
      sessionId
    });

  } catch (error: any) {
    console.error('TV Diminishing Returns import error:', error);
    return NextResponse.json({
      error: 'Import failed',
