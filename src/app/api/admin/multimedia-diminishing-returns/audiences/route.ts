...
export async function GET(request: NextRequest) {
  try {
    ...
    const audiences = await prisma.multimediaTargetAudience.findMany({
      where: {
        countryId,
        businessUnitId
      }
    ...
    console.error('Error fetching Multimedia target audiences:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    await prisma.multimediaTargetAudience.deleteMany({
      where: {
        countryId,
        businessUnitId
      }
    ...
    const createdAudiences = await Promise.all(
      audiences.map(audience =>
        prisma.multimediaTargetAudience.create({
          data: {
            countryId,
            businessUnitId,
    ...
    console.error('Error saving Multimedia target audiences:', error);
    return NextResponse.json(
