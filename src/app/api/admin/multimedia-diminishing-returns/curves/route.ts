...
export async function GET(request: NextRequest) {
  try {
    ...
    const curves = await prisma.multimediaDiminishingReturns.findMany({
      where: {
        countryId,
        businessUnitId
      },
    ...
    console.error('Error fetching Multimedia curves:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    await prisma.multimediaDiminishingReturns.deleteMany({
      where: {
        countryId,
        businessUnitId
      }
    ...
    const createdCurves = await Promise.all(
      curves.map(curve =>
        prisma.multimediaDiminishingReturns.create({
          data: {
            countryId,
            businessUnitId,
            trp: curve.trp,
    ...
    console.error('Error saving Multimedia curves:', error);
    return NextResponse.json(
