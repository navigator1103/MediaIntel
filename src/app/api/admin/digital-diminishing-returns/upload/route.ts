...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate session ID
    const sessionId = `ddr_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    const sessionData = {
      ...
      type: 'digital-diminishing-returns'
    };
    ...
    return NextResponse.json({
      ...
      sessionId,
      totalRecords: records.length,
      message: 'Digital Diminishing Returns data uploaded successfully'
    });

  } catch (error: any) {
    console.error('Digital Diminishing Returns upload error:', error);
    return NextResponse.json({
      error: 'Upload failed',
